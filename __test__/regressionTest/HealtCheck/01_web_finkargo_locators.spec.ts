...
test.describe('Testing screenplay-playwright-js web module', () => {
    test('Navegar por Staging Col', async ({ actor }) => {
        await test.step('Navigate to playwright page', async () => {
            await actor.attemptsTo(
                Navigate.to('https://app-staging.finkargo.com.co/auth/signup'),
            );
            await expect(BrowseTheWeb.as(actor).getPage()).toHaveURL('https://app-staging.finkargo.com.co/auth/signup');
        });
    });
  ...
  //  test('Element.enabled', async ({ actor }) => {
      //  const page: Page = BrowseTheWeb.as(actor).getPage();
        ...
        //await actor.attemptsTo(
          //  Navigate.to('https://app-staging.finkargo.com.co/auth/login'),
         //   Wait.forLoadState('networkidle'),
